...
<Callout>
The LLM Engine automatically scales the number of workers based on the workload, ensuring efficient resource utilization and optimal performance.
</Callout>
...
<Callout>
You can easily extend the LLM Engine to support additional language models by updating the `availableModels` constant and providing the necessary integration code.
</Callout>
...
For detailed information on the LLM Engine's API and advanced usage, refer to the [API documentation](/llm-engine/api-reference).