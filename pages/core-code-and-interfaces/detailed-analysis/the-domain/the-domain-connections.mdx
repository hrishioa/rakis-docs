...
graph TD
  subgraph TheDomain
    TheDomain --> PacketDB
    TheDomain --> InferenceDB
    TheDomain --> EmbeddingEngine
    TheDomain --> LLMEngine
  end

  PacketDB -- Incoming Packets --> P2PNetwork
  PacketDB -- Outgoing Packets --> P2PNetwork
  PacketDB --> InferenceDB

  InferenceDB -- Inference Events --> TheDomain
  InferenceDB -- Embedding Events --> EmbeddingEngine
  InferenceDB -- Inference Events --> LLMEngine

  EmbeddingEngine -- Worker Events --> TheDomain
  LLMEngine -- Worker Events --> TheDomain
```
